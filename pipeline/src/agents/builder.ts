...
- DEPLOY AUTH: NEVER run "vercel login", "netlify login", "sanity login", or any interactive auth command. NEVER call the vercel or netlify CLI directly via shell_exec — ALWAYS use the netlify_deploy tool for static/JAMstack apps (default), or cloud_run_deploy for containerized apps (PHP, Python, apps with persistent backends). For Sanity CLI commands, pass --token $SANITY_AUTH_TOKEN explicitly.
- CLOUD RUN DEPLOY: Use cloud_run_deploy for any stack requiring a Dockerfile (Laravel, Drupal, Django, Express with database, etc). The tool runs gcloud builds submit remotely — no local Docker needed. Always write a Dockerfile in the project_dir before calling cloud_run_deploy. Default port is 8080; match the port your app listens on.`,
...
const ALL_TOOL_NAMES = new Set([
  ...
  'vercel_deploy', 'netlify_deploy', 'firebase_deploy', 'cloud_run_deploy',
